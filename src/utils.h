...
map<string, vector<int>> dictionary_counts() { // construct a dictionary to look for existence of nodes in Tmax
 ...
 // makeproper(Tmax);
  ...
  vector<int> vec(m);
  for (int i = 0; i < m; i++) {
    vec[i] = (Tmax[0][0]->a[encoder[alphabet[i]]]);// no need to encode root node as it always exists, but can have empty context
  }
  ...
  for (int d = 1; d < D + 1; d++) {
    ...
    for (unsigned int k = 0; k < Tmax[d].size(); k++) {
      vector<int> vec2(m);
      for (int i = 0; i < m; i++) {
        vec2[i] = Tmax[d][k]->a[encoder[alphabet[i]]];// no need to encode root node as it always exists, but can have empty context
      }
  ...
  for (int d = 0; d < D + 1; d++) {
    for (unsigned int k = 0; k < T2[d].size(); k++) {
      if (T2[d][k]->leaf == 1) {
        ...
        for (int j = 0; j < m; j++) {
          param[j] = (1.0 * T2[d][k]->a[encoder[alphabet[j]]] + 0.5);
          sum = sum + 1.0 * T2[d][k]->a[encoder[alphabet[j]]] + 0.5;
        }
